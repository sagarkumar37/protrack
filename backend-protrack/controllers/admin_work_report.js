...
exports.masterWeekReport = async(req, res) => {
    try{
        ...
        //raksha
        const plansData =await getWeeklyPlans(startDate, endDate,userName); 
        const tasksData =   await getWeeklyTasks(start_date, end_date, userName);
          
        return res.status(200).send({message:"OK",payload: ratingsData,plansData, tasksData});
...
const getWeeklyPlans = async (startDate, endDate, userNames) =>{
    ...
    SELECT 
    employees.username,
    weekly_plan_reports.activity_id,
    weekly_plan_reports.task_description,
    weekly_plan_reports.status,
    weekly_plan_reports.year_week
    FROM weekly_plan_reports
    LEFT OUTER JOIN employees
    ON employees.id = weekly_plan_reports.employee_id
    WHERE weekly_plan_reports.year_week BETWEEN :startDate AND :endDate
    AND employees.username IN (:userNames)
    ORDER BY 
    year_week, 
    username
